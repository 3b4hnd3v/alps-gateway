...
public class MasterApi {
	...
	public List<Map<String, String>> interfRoute(String intname) {
		...
		try {
			...
			rs = con.execute("/ip/route/print where comment='DefaultRoute"+intname+"'");
	...
	public boolean changeIp(String ident, String name, String ipadd, String network, String defgateway) {
		...
		try {
			...
			String command1 = "/ip/route/add gateway="+defgateway+" comment=Route"+name;
	...
	public String getVNCRule(String interf){
		String ret = "0";
		String df = "DefaultVNC"+interf;
		try {
			ApiConnection con = ApiConnection.connect(ip); // connect to router
			con.login(user,pass);
			List<Map<String,String>> rs = con.execute("/ip/firewall/nat/print where comment='"+df+"'");
	...
	public Map<String, String> wanRoute(String intname, String compr) {
	...
	//QOS
	public String getSimple(String service){
		String ret = "0";
		String df = "Default" + service + "QoS";
		try {
			ApiConnection con = ApiConnection.connect(ip); // connect to router
			con.login(user,pass);
			List<Map<String,String>> rs = con.execute("/queue/tree/print where comment='"+df+"'");
			if(rs.size() > 0) {
				Map<String, String> r = rs.get(0);
				//System.out.print(r);
				ret = r.get(".id");
	...
	public boolean changeSimple(String status, String ident){
		boolean res = false;
		String command = "";
		try {
			ApiConnection con = ApiConnection.connect(ip); 
			...
			command = "/queue/tree/set disabled='"+status+"' .id="+ident;
	...
	public String getQOSId(String service){
		String ret = "0";
		String df = "Default" + service + "QoS";
		try {
			ApiConnection con = ApiConnection.connect(ip); // connect to router
	...
	public Map<String, String> getFullQOS(String service){
	...
	public boolean changeQos(String lim, String max, String priority, String ident){
		boolean res = false;
		String command = "";
		try {
			ApiConnection con = ApiConnection.connect(ip); 
			...
			command = "/queue/tree/set limit-at='"+lim+"' max-limit='"+max+"' priority='"+priority+"' .id="+ident;
	...
	public boolean changeQos(String stat, String max, String ident){
		boolean res = false;
		String command = "";
		try {
			ApiConnection con = ApiConnection.connect(ip); 
			...
			command = "/queue/tree/set disabled='"+stat+"' max-limit='"+max+"' .id="+ident;
	...
	public Map<String, String> getFullP2P(){
		Map<String, String> r = new HashMap<String, String>();
		String df = "DefaultLimitP2P";
		try {
			ApiConnection con = ApiConnection.connect(ip); // connect to router
			con.login(user,pass);
			List<Map<String,String>> rs = con.execute("/queue/simple/print where comment='"+df+"'");
			if(rs.size() > 0) {
				r = rs.get(0);
			}
			con.close();
	...
	public boolean changeP2P(String newStatus, String ident){
		boolean res = false;
		String command = "";
		try {
			ApiConnection con = ApiConnection.connect(ip); 
			...
			command = "/queue/simple/set disabled='"+newStatus+"' .id="+ident;
