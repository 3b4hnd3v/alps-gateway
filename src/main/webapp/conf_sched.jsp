...
<%! public String url = "SELECT * FROM `mr_purchase_line` ORDER BY `purchase` DESC", gid="", gnm="", lnm="", gpc="", cvp="collapse";%>
<%
if(request.getParameter("q") != null && request.getParameter("q").equals("delete")&& request.getParameter("id")!= null) {
	...
	try {
        ...
        if(cn1.createStatement().execute(deleteSQL)){
    		...
    		al.addLog(logact);
    		response.sendRedirect("conf_schedule.jsp?msg=Delete Succesful&type=success");
        }
        cn1.close();
		
	} catch (Exception e1) { 
		System.out.println(e1); 
		response.sendRedirect("conf_schedule.jsp?error=Delete Not Successful");
		}
}
%>
                 ...
                 <table id="example1" class="table table-bordered table-striped">
                   ...
                   <tbody>
                   	 ...
                   	 try {
						   ...
						   String schbtn="<a href='room_sched.jsp?&mrid="+rs.getString("meeting_room_id")+"&mrn="+rs.getString("meeting_room")+"' target='_self'><button class='btn btn-xs btn-primary'>All Sched</button></a>";
						   ...
						   out.println("<td>"+dlbtn+"	"+schbtn+"</td>");
