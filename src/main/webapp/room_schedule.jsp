...
if(request.getParameter("q") != null && request.getParameter("q").equals("delete")&& request.getParameter("id")!= null) {
	...
	try {
        ...
        if(cn1.createStatement().execute(deleteSQL)){
    		...
    		al.addLog(logact);
    		response.sendRedirect("room_schedule.jsp?msg=Delete Succesful");
        }
        cn1.close();
		
	} catch (Exception e1) { 
		System.out.println(e1); 
		response.sendRedirect("room_schedule.jsp?error=Delete Not Successful");
		}
}
%>
<%
if(request.getParameter("mrid") != null) {
	...
	try {
		...
		url = "SELECT * FROM `mr_purchase_line` WHERE `meeting_room_id`="+mrid+" ORDER BY `start_time` DESC";
